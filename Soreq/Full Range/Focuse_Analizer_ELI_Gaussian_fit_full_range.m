...
for ii =3:length(Dir)
    ...
    Background = mean(mean(Image(1:1000,[1:500,2200:end])));
    % end
    % Image(Image<13500) = Background; %13500
    % Background = 13500;
    ...
    [center_x(i), f_x{i},f_cont_x{i},pix_vec_fit_x] = Gaussian_fit (vec_x, I_x,x0);
    [center_y(i), f_y{i},f_cont_y{i},pix_vec_fit_y] = Gaussian_fit (vec_y, I_y,y0);
    ...
    [fwhm_x_mu(i),fwhm_x_pxl(i),e_minus2_x_mu(i),e_minus2_x_pxl(i)] = FWHM_and_2_over_e_2_fun_1D(x_microns{i},f_x{i});
    [fwhm_y_mu(i),fwhm_y_pxl(i),e_minus2_y_mu(i),e_minus2_y_pxl(i)] = FWHM_and_2_over_e_2_fun_1D(y_microns{i},f_y{i});
...
function [center,f,f_con,pix_vec_] = Gaussian_fit (pix_vec, I,x0)
 % x0 = [max(I_x),Max_x_index,1,0];
pix_vec = reshape(pix_vec,size(I));
Vf = @(x) (pix_vec-x(2)) / x(3);
% Bessel_fun = @(x) x(1) *(besselj(1,Vf(x)) ./ Vf(x)).^2 + x(4);

Gauss_fun = @(x) x(1) * exp(-Vf(x).^2) + x(4);

err = @(x) sum((Gauss_fun(x) - I).^2);
...
pix_vec_ = linspace(pix_vec(1),pix_vec(end),1001);
vf = (pix_vec_-x(2)) / x(3);
f_con =  x(1) * exp(-vf.^2) + x(4);
...
function [fwhm_mu,fwhm_pxl,e_minus2_mu,e_minus2_pxl] = FWHM_and_2_over_e_2_fun_1D(microns,f)
% f = f-min(f);
% f = f/max(f);
len_f_old = length(f);
min_i_pxl = find(f==min(f));  min_i_pxl=min_i_pxl(1);
...
while Ratio<=0.7
    ...
    if radius_pxl> min([rows/2,cols/2])
